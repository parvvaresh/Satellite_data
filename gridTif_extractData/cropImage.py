...
class CropImage:
    def __init__(self, path_tif_file: str, size_crop_block: int, size_crop_pixle: int, path_to_save: str) -> None:
        ...
        self.size_crop_block = size_crop_block
        self.size_crop_pixle = size_crop_pixle
        
        make_folder(self.root_dir)
        print("✅ Created root folder")
        
        self.block_dir = os.path.join(self.root_dir, f"{self.size_crop_block}_{self.size_crop_block}")       
        make_folder(self.block_dir)
    ...
    def _crop_tiff(self, input_filename: str, output_file: str, x_off: int, y_off: int, x_size: int, y_size: int) -> bool:
        """
    ...
    def _crop_image(self, input_filename: str, output_filename: str, out_path: str, crop_size: int, xsize: int, ysize: int) -> None:
        """
    ...
    def _handle_pixle(self) -> None:
        all_tif_files = get_all_tif_files(self.block_dir)
        ...
        for path, name in all_tif_files:
            ...
            self._crop_image(path, f"{id}_", self.pixle_dir, self.size_crop_pixle, xsize, ysize)
    ...
    def pipeline_crop(self) -> None:
        ...
        self._crop_image(self.path_tif_file, "", self.block_dir, self.size_crop_block, xsize, ysize)
        self._handle_pixle()
